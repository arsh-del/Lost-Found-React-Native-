    ...
    useEffect(()=>{
        const fetchData =async ()=>{
            ...
            const response = await axios.get(
                'http://10.0.2.2:8080/lostItems/'+route.params.id
            );
    ...
    return (
        <View style={styles.center}>
            ...
            <View>
                ...
                <TextInput style={{fontSize:28}} placeholder={found.name} value={name}
                           onChangeText={(text) => setName(text)} />
                <TextInput placeholder={found.description} style={{fontSize:22}} value={description}
